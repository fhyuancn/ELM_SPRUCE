...
!
   ...
   real(r8), pointer :: npool(:)      ! (gN/m2) plant N pool (PET addition)
...
!
   ...
   real(r8), pointer :: excess_cflux_to_cpool(:)
   ...
   real(r8), pointer :: cpool_to_turnover(:)
...
!-----------------------------------------------------------------------
   ...
   psnsun                      => pcf%psnsun
   psnsha                      => pcf%psnsha
   excess_cflux_to_cpool       => pcf%excess_cflux_to_cpool
   cpool_to_turnover           => pcf%cpool_to_turnover
   if ( use_c13 ) then
...
!leafcn(ivt(p))= 0.0097*mon**5-0.2958*mon**4+3.131*mon**3-13.681*mon**2+25.085*mon+20.318
leafcn(ivt(p))= 0.0101*mon**5-0.3039*mon**4+3.1563*mon**3-13.48*mon**2+24.746*mon+17.273
...
!               (ivt(pfti(c)) >= npcropmin)) ) then
   ...
   do fp=1,num_soilp
...
#ifdef NPOOLMOD
      !DMR 5/17/2011 - add PET changes from 2/1/2011 for plant N pool
      !                Purpose is to buffer against diurnal changes in GPP due to N 
      !                downregulation
      !rc = 10.0_r8
      rc = 3*max(annsum_npp(p) * n_allometry(p) / c_allometry(p), 0.01_r8)
...
#ifdef CPOOLMOD
      excess_cflux_to_cpool(p) = excess_cflux(p)  !DMR addition
#endif
      ...
      if (ivt(p) >= npcropmin) then ! skip 2 generic cropss
         cpool_to_livestemc(p)          = nlc * f3 * f4 * fcur
         cpool_to_livestemc_storage(p)  = nlc * f3 * f4 * (1._r8 - fcur)
         cpool_to_deadstemc(p)          = nlc * f3 * (1._r8 - f4) * fcur
         cpool_to_deadstemc_storage(p)  = nlc * f3 * (1._r8 - f4) * (1._r8 - fcur)
...
#ifdef CPOOLMOD
      cpool_to_turnover(p) = (cpool(p) + excess_cflux_to_cpool(p)*dt - plant_calloc(p) * dt)  * 0.66/(86400._r8 * 60._r8)
      !                       (1._r8 - exp(-dt/(86400._r8*30._r8)))/dt !DMR
      !cpool = cpool(p) + excess_cflux_to_cpool * dt - cpool_to_turnover(p) * dt
#endif
