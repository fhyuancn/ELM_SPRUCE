...
!-----------------------------------------------------------------------
   ...
   ! assign local pointers to derived type arrays (out)
    ...
    leafn_to_npool                 => pnf%leafn_to_npool
    frootn_to_litter               => pnf%frootn_to_litter
    frootn_to_npool                => pnf%frootn_to_npool
...
#ifdef NPOOLMOD
         leafn_to_npool(p) = (leafc_to_litter(p) / leafcn(ivt(p))) - leafn_to_litter(p)
#else
...
#ifdef NPOOLMOD
         livestemn_to_npool(p)  = ntovr - livestemn_to_deadstemn(p)
#else
...
#ifdef NPOOLMOD
         livecrootn_to_npool(p)  = ntovr - livecrootn_to_deadcrootn(p)
#else
