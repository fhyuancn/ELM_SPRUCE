...
#endif
#if (defined MICROBE)
  use microbevarcon       , only : usephfact, fin_use_fsat
#endif
...
!
  ...
  real(r8), pointer :: p3(:)              ! coefficient for determining finundated (m)
  real(r8), pointer :: pH(:) 
  real(r8), pointer :: soilpH_unsat(:,:) 
  real(r8), pointer :: soilpH_sat(:,:) 
#endif
...
#ifdef MICROBE
  allocate(pH(begg:endg))
  allocate(soilpH_unsat(begg:endg, 1:nlevgrnd))
  allocate(soilpH_sat(begg:endg, 1:nlevgrnd))
#endif
...
! def CH4
	  ...
	  ! Microbe Methane code parameters for finundated
#ifdef MICROBE
  if (.not. fin_use_fsat) then
     call ncd_io(ncid=ncid, varname='ZWT0', flag='read', data=zwt0_in, dim1name=grlnd, readvar=readvar)
     if (.not. readvar) then
     ...
     call ncd_io(ncid=ncid, varname='F0', flag='read', data=f0_in, dim1name=grlnd, readvar=readvar)
     if (.not. readvar) then
        call endrun( trim(subname)//' ERROR: Running with microbial CH4 Model but F0 not on surfdata file')
     end if
     call ncd_io(ncid=ncid, varname='P3', flag='read', data=p3_in, dim1name=grlnd, readvar=readvar)
     if (.not. readvar) then
   ...
   do c = begc, endc
	...
	soilpH_sat(c,lev) = 7
      ...
      if(h2osfcflag == 0) then 
         ...
         h2osfc_thresh(c) = 10._r8 !!! 12,120
#if (defined HUM_HOL)
      h2osfc_thresh(c) = 300._r8
#endif
