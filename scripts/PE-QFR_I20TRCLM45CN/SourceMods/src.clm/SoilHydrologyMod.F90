...
!
    ...
    real(r8), pointer :: h2osfc(:)         ! surface water (m)
    ...
    do fc = 1, num_hydrologyc
       ...
       if (ltype(clandunit(c)) == istsoil .or. ltype(clandunit(c))==istcrop) then
          ...
          if (c .eq. 1) then
             ...
             !qflx_surf(1) = qflx_surf(1) + qflx_infl_excess(c)    !DMR 4/29/13
             !qflx_surf_input(2) = qflx_surf_input(2) + qflx_infl_excess(c)
         ...
         if(h2osfcflag==0) then 
          ...
          if (jwt(c) .lt. nlevsoi) then
             do j=nlevsoi,jwt(c)+1,-1
               ...
               s1 = min(1._r8, s1)
               if (c .eq. 1) ka_hu = ka_hu+(hksat(c,j)*s1**(2._r8*bsw(c,j)+3._r8))*dzmm(c,j)/sum(dzmm(c,jwt(c)+1:nlevsoi))
               if (c .eq. 2) ka_ho = ka_ho+(hksat(c,j)*s1**(2._r8*bsw(c,j)+3._r8))*dzmm(c,j)/sum(dzmm(c,jwt(c)+1:nlevsoi))
             end do
          else
...
!
    ...
    real(r8), pointer :: qcharge(:)        ! aquifer recharge rate (mm/s)
#if (defined HUM_HOL)
    real(r8), pointer :: qflx_lat_aqu(:)   ! lateral transport in aquifer
    real(r8), pointer :: qflx_lat_aqu_layer(:,:)
    real(r8), pointer :: qflx_surf_input(:) ! input from surface runoff (from hummock to hollow) (mmH2O/s)
#endif
...
!    real(r8) :: qflx_lat_aqu_layer(lbc:ubc:1:nlevsoi)
    !
    real(r8) :: theta_unsat
    real(r8) :: f_unsat
    real(r8) :: s_y
...
#else
       rsub_top(c)    = imped * rsub_top_max* exp(-fff(c)*zwt(c)) !(HUM_HOL !comment out by XS)
       !write(iulog,*) "RSUB_TOP", rsub_top(c), rsub_top_max, zwt(c)
#endif
...
!-- recompute jwt  ---------------------------------------------------------
    ...
    do fc = 1, num_hydrologyc
       ...
       if (ltype(clandunit(c)) == isturb) then
	     ...
	     !TEST - reduce infiltration by the same amount since this "infiltration" didn't really happen
	     qflx_infl(c) = qflx_infl(c) - xs1(c) / dtime
             qflx_rsub_sat(c)     = 0._r8
          else
