...
!------------------------------------------------------------------------------
   ...
   h2o_moss_inter => pws%h2o_moss_inter
   leafc          => pcs%leafc 
   dt_veg         => pps%dt_veg
   sabv           => pef%sabv
...
#else
         if (vol_liq .le. 0._r8 .or. t_soisno(c,j) .le. tfrz      ) then
#endif
            rootr(p,j) = 0._r8
         else
...
!KO
         ...
         if(ivt(p) == 12) then    
            ...
            !write(iulog,*)'h2o_moss_wc=',h2o_moss_wc(p),'leafc=',leafc(p)
         end if     
      end do
...
!         + cir(p)*t_grnd(c)**4 - eflx_sh_veg(p) - hvap*qflx_evap_veg(p)
      ...
      !write(iulog,*)'h2ocan=',h2ocan(12),'qflx_tran_veg=',qflx_tran_veg(12),'qflx_evap_veg=',qflx_evap_veg(12)
      ! Need a gobal variable for h2ocanmx and check if h2ocan > h2ocanmx, then
      ! the excess water will go soil if there is no veg.
...
! local pointers to implicit in variables
   ...
   integer , pointer :: ivt(:)      ! pft vegetation type
   integer , pointer :: pcolumn(:)  ! pft's column index
      ! Fenghui Yuan 06-28-2021
     integer  :: year             ! year (0, ...) for nstep+1
    integer  :: month              ! month (1, ..., 12) for nstep+1
    integer  :: day              ! day of month (1, ..., 31) for nstep+1
    integer  :: secs              ! seconds into current date for nstep+1
   real(r8), pointer :: forc_pbot(:)! atmospheric pressure (Pa)
   real(r8), pointer :: t_veg(:)    ! vegetation temperature (Kelvin)
   real(r8), pointer :: btran(:)    ! transpiration wetness factor (0 to 1)
...
!KO   tpuc   = fth25 (tpuhd, tpuse)
   ...
   do f = 1, fn
      ...
      if (c3flag(p)) then
         ...
         theta_cj(p) = 0.80_r8
         bbbopt(p) = 40000._r8
         mbbopt(p) = mp(ivt(p)) !4._r8
      end if

      ! Soil water stress applied to Ball-Berry parameters
      if (ivt(p) == 12) then
         bbb(p) = (-0.195 + 0.134*(h2o_moss_wc(p)+1._r8) - 0.0256*(h2o_moss_wc(p) + 1.0_r8)**2  &
   ...
   ! Fenghui Yuan 06-28-2021
    ...
    !leafcn(ivt(p))= 0.0097*month**5-0.2958*month**4+3.131*month**3-13.681*month**2+25.085*month+20.318
    leafcn(ivt(p))= 0.0101*month**5-0.3039*month**4+3.1563*month**3-13.48*month**2+24.746*month+17.273
   do f = 1, fn
      p = filterp(f)
...
!KO
            vcmaxse = 679.87_r8 - 0.00_r8 * min(max((t10(p)-tfrz),11._r8),35._r8)
            jmaxse  = 679.87_r8 - 0.00_r8 * min(max((t10(p)-tfrz),11._r8),35._r8)
            tpuse = vcmaxse
...
!KO
            ...
            if (ivt(p) == 12) then
              ci_z(p,iv) = cair(p)-an(p,iv) * forc_pbot(g)/gs_mol(p,iv)
            endif
...
! subroutine hybrid and brent in this module
   ...
   if (ivt(p) == 12) fval = ci - cair + an(p,iv) * forc_pbot(g)/gs_mol 
